...
class Amfphp_Core_Gateway {
    ...
    public function  __construct(array $getData, array $postData, $rawInputData, $contentType, Amfphp_Core_Config $config = null) {
        ...
        //short for Name
        $rawInputDataArray = json_decode($rawInputData,true);
        
        $isJson = true;
        if (NULL == $rawInputDataArray) {
        	$isJson = false;
        }
        if (empty($rawInputDataArray['serviceName'])) {
            $rawInputDataArray['serviceName'] = "FFService";
        }
        if (!$isJson) {
        	$rawInputDataArray['methodName'] = "FF";
        }else{
        	$rawInputDataArray['methodName'] = "exec";
        }
        if (!empty($rawInputDataArray["C"]) || !empty($rawInputDataArray["P"])) {
            $rawInputDataArray["P"][0]['cmdName'] = $rawInputDataArray["C"];
            if (!empty($rawInputDataArray['F'])) {
                $rawInputDataArray["P"][0]['funName'] = $rawInputDataArray["F"];
                unset($rawInputDataArray["F"]);
            }
            $rawInputDataArray['parameters'] = $rawInputDataArray["P"];
            unset($rawInputDataArray["P"],$rawInputDataArray["C"]);
        }
        
        $rawInputData = json_encode($rawInputDataArray);
